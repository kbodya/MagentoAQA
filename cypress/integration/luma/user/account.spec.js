...
describe('Account test creation', () => {
    it('Can create an account', () => {
        ...
        cy.contains('Thank you for registering with Code8 Beauty.').should('exist')
        cy.wait(2000)
        Account.logout()
    })
})
...
describe('Account activities', () => {
    ...
    it.skip('Can check your profile and change password', () => {
        cy.visit(account.routes.accountEdit)
        cy.get('input[name="change_password"]').check({force: true})
    ...
    it.skip('Can view order history', () => {
    ...
    it('Can add an address automatically from saved address', () => {
        ...
        cy.get('button[title="Add to Bag"]').click(({ multiple: true, force: true }))
        cy.wait(3000)
        ...
        // cy.get ('div > div.header-block.right > div.header-profile > a').click ()
        // cy.visit('/')
        cy.wait(3000)
        // cy.get('.page-header .customer-welcome > .customer-name > .action').click()
        cy.get ('.header-profile__link > span').click ()
        cy.contains('Log Out').click()
    })
    ...
    it.skip('Can change the newsletter subscription', () => {
        Account.login(account.customer.customer.email, account.customer.password)
    ...
    it.skip('Can add a product the a wishlist', () => {
        cy.visit(product.simpleProductUrl)
        cy.wait(2000)
    ...
    it('Can log out', () => {
        ...
        cy.get(selectorsLuma.accountMenuItems).contains('Log Out').click({force: true})
